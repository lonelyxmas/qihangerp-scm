...
import cn.qihang.ai.assistant.service.CodePiService;
import cn.qihang.ai.assistant.service.ConfigService;
import cn.qihang.ai.assistant.service.FeishuCodingBotService;
...
public class CodingController {
    ...
    public Map<String, Object> getRecords(@RequestParam(defaultValue = "50") int limit) {
        ...
        log.debug("[Coding] 查询记录: limit={}, total={}, returned={}", limit, all.size(), to);
    ...
    public Map<String, Object> debug(@RequestBody Map<String, String> body) {
        ...
        if (message == null || message.trim().isEmpty()) {
            log.warn("[Coding] 手动调试: message 为空");
        ...
        debugExecutor.execute(() -> {
            try {
                ...
                if (entity != null) {
                    ...
                    log.info("[Coding] 记录已更新: id={}, success={}, duration={}s",
                            finalRecordId, entity.getSuccess(), entity.getDuration());
                }

                logService.add("代码调试", result.isSuccess() ? "成功" : "失败",
                        String.format("耗时%s | dir=%s | recordId=%d",
                                result.getElapsedStr(), finalProjectDir, finalRecordId));
            } catch (Exception e) {
                log.error("[Coding] 后台排查异常: recordId={}, err={}", finalRecordId, e.getMessage(), e);
                CodingRecordEntity entity = codingBotService.getRecordById(finalRecordId);
                if (entity != null) {
    ...
    public Map<String, Object> testFeishu() {
        ...
        if (chatId == null || chatId.isEmpty()) {
            log.warn("[Coding] 测试消息: 群 Chat ID 未配置");
        ...
        if (appId == null || appId.isEmpty() || appSecret == null || appSecret.isEmpty()) {
            log.warn("[Coding] 测试消息: App ID/Secret 未配置");
            return Map.of("ok", false, "error", "请先配置 App ID 和 App Secret");
        }

        log.info("[Coding] 测试消息: chatId={}, appId={}...", chatId,
                appId.length() > 8 ? appId.substring(0, 8) + "..." : appId);

        if (!codingBotService.isConnected()) {
            log.info("[Coding] 测试消息: WebSocket 未连接，尝试启动");
            try {
                codingBotService.startLongConnection(appId, appSecret);
                Thread.sleep(2000);
            } catch (Exception e) {
                log.error("[Coding] 测试消息: 启动失败: {}", e.getMessage());
                return Map.of("ok", false, "error", "启动机器人失败: " + e.getMessage());
            }
        }

        if (!codingBotService.isConnected()) {
            log.warn("[Coding] 测试消息: WebSocket 仍在启动中");
            return Map.of("ok", false, "error", "机器人正在启动连接中，请稍后重试");
        }
        ...
        try {
            ...
            codingBotService.sendTestMessage(chatId,
                    "✅ 连接测试成功！\n\n机器人已就绪，现在可以在群中 @我 并描述你遇到的代码问题，我将自动排查。\n\n⏰ 当前时间: " + now);
            logService.add("代码排查测试", "成功", "已向群 " + chatId + " 发送测试消息");
            log.info("[Coding] ✅ 测试消息发送成功: chatId={}", chatId);
            return Map.of("ok", true, "message", "测试消息已发送，请检查飞书群");
        } catch (Exception e) {
            log.error("[Coding] ❌ 测试消息发送失败: {}", e.getMessage());
            logService.add("代码排查测试", "失败", e.getMessage());
