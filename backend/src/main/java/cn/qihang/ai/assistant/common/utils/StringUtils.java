...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    public static String format(String template, Object... params) {
        ...
        for (Object param : params) {
            template = template.replaceFirst("\\{\\}", param == null ? "" : param.toString());
        }
        return template;
    }
    ...
    public static boolean containsAny(Collection<String> collection, String... array) {
        ...
        for (String str : array) {
            if (collection.contains(str)) return true;
        }
        return false;
    }

    public static boolean containsAnyIgnoreCase(CharSequence cs, CharSequence... searchCharSequences) {
        if (isEmpty(cs) || isEmpty(searchCharSequences)) return false;
        for (CharSequence testStr : searchCharSequences) {
            if (containsIgnoreCase(cs, testStr)) return true;
        }
        return false;
    }
    ...
    public static String toCamelCase(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            if (c == SEPARATOR) upperCase = true;
            else if (upperCase) { sb.append(Character.toUpperCase(c)); upperCase = false; }
            else sb.append(c);
        }
        return sb.toString();
    }
    ...
    public static final String padl(final String s, final int size, final char c) {
        ...
        if (s != null) {
            ...
            if (s.length() <= size) {
                for (int i = size - len; i > 0; i--) sb.append(c);
                sb.append(s);
            } else {
                return s.substring(len - size, len);
            }
        } else {
            for (int i = size; i > 0; i--) sb.append(c);
        }
        return sb.toString();
    }
